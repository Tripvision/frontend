...
export const deleteAndUpdateByTaskId = createAsyncThunk(
  "files/delete",
  async ({ id, taskId, formData }, thunkAPI) => {
    try {
      console.log(id);
      console.log(taskId);
      console.log(JSON.stringify(formData.get("fileList")));
      const response = await fileService.deleteProjectFile(id, taskId);
      const postResponse = await fileService.createProjectFile(
        id,
        taskId,
        formData
      );
